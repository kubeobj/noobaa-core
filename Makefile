...
builder: assert-container-engine
	...
	$(CONTAINER_ENGINE) build $(CONTAINER_IMAGE_BUILDER_ARGS) $(CONTAINER_PLATFORM_FLAG) $(CPUSET) --build-arg CENTOS_VER=$(CENTOS_VER) --build-arg BUILD_S3SELECT=$(BUILD_S3SELECT) --build-arg BUILD_S3SELECT_PARQUET=$(BUILD_S3SELECT_PARQUET) -f src/deploy/NVA_build/builder.Dockerfile $(CACHE_FLAG) $(NETWORK_FLAG) -t $(BUILDER_TAG) .
	$(CONTAINER_ENGINE) tag $(BUILDER_TAG) noobaa-builder
	@echo "##\033[1;32m Build image noobaa-builder done.\033[0m"
.PHONY: builder

base: builder
	@echo "\n##\033[1;32m Build image noobaa-base ...\033[0m"
	$(CONTAINER_ENGINE) build $(CONTAINER_IMAGE_BUILDER_ARGS) $(CONTAINER_PLATFORM_FLAG) $(CPUSET) --build-arg BUILDER_TAG=$(BUILDER_TAG) --build-arg BUILD_S3SELECT=$(BUILD_S3SELECT) --build-arg BUILD_S3SELECT_PARQUET=$(BUILD_S3SELECT_PARQUET) -f src/deploy/NVA_build/Base.Dockerfile $(CACHE_FLAG) $(NETWORK_FLAG) -t $(NOOBAA_BASE_TAG) . $(REDIRECT_STDOUT)
	$(CONTAINER_ENGINE) tag $(NOOBAA_BASE_TAG) noobaa-base
...
noobaa: base
	...
	$(CONTAINER_ENGINE) build $(CONTAINER_IMAGE_BUILDER_ARGS) $(CONTAINER_PLATFORM_FLAG) $(CPUSET) --build-arg NOOBAA_BASE_TAG=$(NOOBAA_BASE_TAG) --build-arg CENTOS_VER=$(CENTOS_VER) --build-arg BUILD_S3SELECT=$(BUILD_S3SELECT) --build-arg BUILD_S3SELECT_PARQUET=$(BUILD_S3SELECT_PARQUET) -f src/deploy/NVA_build/NooBaa.Dockerfile $(CACHE_FLAG) $(NETWORK_FLAG) -t $(NOOBAA_TAG) --build-arg GIT_COMMIT=$(GIT_COMMIT) . $(REDIRECT_STDOUT)
	$(CONTAINER_ENGINE) tag $(NOOBAA_TAG) noobaa
